...
function boxes = detect_MM(im, model,thresh,partIDs, min_level) %%Hamed
...
%     boxes_t  = zeros(100000,numparts*4+2);
    boxes_t  = zeros(100000,numparts*4+2+1); %%Hamed
...
%         boxes_t(i,:) = [box repmat(c,length(I),1) score(I)];
        boxes_t(i,:) = [box repmat(c,length(I),1) repmat(rlevel,length(I),1) score(I)]; %%Hamed
        cnt = i(end);
      end
    end
